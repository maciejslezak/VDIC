...
//------------------------------------------------------------------------------
	...
	task send_op(input operation_t iop, input st_data_in_packet_t idata_in_packet);
		...
		while (1) begin : sender_loop
			...
			priority if (busy_out == 1'b0 && valid_counter == 1'b0) begin
				...
				valid_counter       = 1'b0;
				data_in_valid       = 1'b1;
				command_q.push_front('{data_in_packet,op});
				@(negedge clk);
...
always @(posedge clk) begin : op_monitor
	if (!rst_n) begin
		...
		command_monitor_h.write_to_monitor(command_q.pop_back());
end
...
initial begin : result_monitor_thread
	st_data_out_packet_t	data_out_packet;
    forever begin
        @(posedge clk);
        if (data_out_valid) begin
	        data_out_packet.data_out				= data_out;
	        data_out_packet.data_out_parity			= data_out_parity;
	        data_out_packet.data_in_parity_error	= data_in_parity_error;
            result_monitor_h.write_to_monitor(data_out_packet);
        end
    end
